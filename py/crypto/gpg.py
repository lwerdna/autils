...
def decrypt_cfb(ctext, key):
	...
	FR = ctext[0:0+8]
	FRE = cast128_encrypt_block(FR, key)
	check = dataXor(ctext[8:8+2], FRE[0:0+2])
...
if __name__ == "__main__":
	...
	# read file, assume its data (non-ascii)
	fobj = open(sys.argv[1])
	data = fobj.read()

	# if "ascii armor" (base64) then decode
	if re.search(r'-----BEGIN PGP MESSAGE-----', data[0:64]):
		fobj.seek(0)
